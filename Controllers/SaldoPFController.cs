...
namespace InPay__CuriousCat_BackEnd.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class SaldoPFController : ControllerBase
    {
        private readonly AppDbContext _context;

        public SaldoPFController(AppDbContext context)
        {
            _context = context;
        }
        [HttpGet("{conta}")]
        public async Task<ActionResult<decimal>> GetSaldo(string conta)
        {
            ...
            try
            {
                ...
                // Executa o UPDATE
                var updateSql = $@"
            UPDATE contas_pessoas_fisica AS cpf 
            SET saldo = (
                SELECT SUM( 
                    CASE
        ...
        ";
                ...
                // Executa o SELECT
                var saldo = await _context.contas_pessoas_fisica
                    .FromSqlRaw($"SELECT numero_conta, coalesce(CAST(saldo AS numeric),0) as saldo FROM contas_pessoas_fisica AS cpf WHERE cpf.numero_conta = {numeroConta}")                    
                    .FirstOrDefaultAsync();
